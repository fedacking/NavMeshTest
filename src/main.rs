...
fn cs(map_number: f64) -> f32 { (map_number / 24.0 + 500.0) as f32 }

fn draw_nav_triangle(triangle: &Triangle, color: Color) {
    let [l1, l2, l3] = triangle.to_array();
    let v1 = Vec2 { x: cs(l1.x), y: cs(l1.y) };
    let v2 = Vec2 { x: cs(l2.x), y: cs(l2.y) };
    let v3 = Vec2 { x: cs(l3.x), y: cs(l3.y) };
...
async fn main() {
    let file_data = read_file("test_input/merliot.fnav").unwrap();
