...
impl DelaunayTriangulation {
    ...
    fn triangulate(&mut self, points: &Vec<Vec2>) {
        ...
        for triangle in &self.triangles {
            self.edges
                .insert(Edge::new(triangle.coordinates[0], triangle.coordinates[1]));
            self.edges
                .insert(Edge::new(triangle.coordinates[1], triangle.coordinates[2]));
            self.edges
                .insert(Edge::new(triangle.coordinates[2], triangle.coordinates[0]));
        }
    }

    fn check_edges(&self, edge: Edge) -> Option<Vec2> {
        None
    }
}
